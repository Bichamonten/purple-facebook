...
fb_cb_api_messages(FbApi *api, GSList *msgs, gpointer data)
{
	...
	autoo = purple_account_get_bool(acct, "auto-read", TRUE);
...
static gboolean
fb_cb_conv_autoread(gpointer data)
{
	const gchar *name;
	FbData *fata;
	FbId id;
                   ...
                   gpointer data)
{
	...
	if ((type == PURPLE_CONVERSATION_UPDATE_UNSEEN) &&
	    purple_strequal(pid, FB_PROTOCOL_ID) &&
	    purple_account_get_bool(acct, "mark-auto", TRUE))
	{
		/* Use event loop for purple_conversation_has_focus() */
		name = purple_conversation_get_name(conv);
		tname = g_strconcat("conv-read-", name, NULL);
		fb_data_add_timeout(fata, tname, 1, fb_cb_conv_autoread, conv);
		g_free(tname);
	}
}
...
purple_init_plugin(PurplePlugin *plugin)
{
	...
	opt = purple_account_option_bool_new(_("Auto mark messages as read when received"),
	                                     "mark-auto", TRUE);
	opts = g_list_prepend(opts, opt);
